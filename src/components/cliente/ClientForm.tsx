    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          className="w-full glass-input rounded-2xl px-4 py-3 text-sm font-semibold border-slate-200 focus:border-indigo-500"
          required
          autoComplete="off"
          disabled={isLoading}
        />
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-650 text-xs font-black select-none">+51</span>
          <input
            type="text"
            value={telefono}
            ...
            maxLength={9}
            className={`w-full glass-input rounded-2xl pl-12 pr-4 py-3 text-sm font-mono font-bold border-slate-200 ${
              phoneError ? "border-rose-300 focus:border-rose-500" : "focus:border-indigo-500"
            }`}
            autoComplete="off"
            disabled={isLoading}
          />
        ...
        {phoneError ? (
          ...
          <p className="text-[10px] text-slate-500 mt-1 font-semibold">9 dígitos celulares (el +51 se añade automáticamente)</p>
        )}
      </div>
      ...
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block flex items-center gap-1.5">
          <MapPin size={10} /> Dirección <span className="text-slate-400 normal-case font-medium">(opcional)</span>
        </label>
        <input
          type="text"
          value={direccion}
      ...
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block flex items-center gap-1.5">
          <CreditCard size={10} /> Cuenta Bancaria <span className="text-slate-400 normal-case font-medium">(opcional)</span>
        </label>
        <input
          type="text"
          value={numeroCuenta}
          ...
          className="w-full glass-input rounded-2xl px-4 py-3 text-xs font-mono font-bold border-slate-200 focus:border-indigo-500"
          autoComplete="off"
          disabled={isLoading}
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full glass-input rounded-2xl px-4 py-3 text-xs font-semibold border-slate-200 focus:border-indigo-500"
            disabled={isLoading}
          />
        </div>
        <div className="space-y-1">
      ...
      {!isEdit && (
        <div className="space-y-2 pt-2 border-t border-slate-200/80">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Documentos Soporte</span>
            <div className="flex-1 h-px bg-slate-200" />
          </div>
          ...
          <div className="flex gap-2 items-center">
            <select
              ...
              className="flex-1 glass-input rounded-xl px-3 py-2 text-[10px] font-black cursor-pointer bg-white border-slate-200"
              disabled={isLoading}
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="px-3.5 py-2 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 transition text-[11px] font-black flex items-center gap-1.5 whitespace-nowrap cursor-pointer"
            >
          ...
          {pendingDocs.length > 0 ? (
              ...
              {pendingDocs.map((pd, idx) => (
                <div key={idx} className="flex items-center gap-2 p-2 bg-slate-50 border border-slate-200 rounded-xl">
                  {pd.preview ? (
                    ...
                    <div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center text-sm shrink-0">
                      {TIPOS_DOCUMENTO_CONFIG[pd.tipo].icon}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-slate-800 truncate leading-none">{pd.file.name}</p>
                    <p className="text-[9px] text-indigo-650 font-bold mt-1 uppercase tracking-wider">{TIPOS_DOCUMENTO_CONFIG[pd.tipo].label}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => removePendingDoc(idx)}
                    className="p-1.5 rounded-lg hover:bg-rose-50 text-slate-500 hover:text-rose-600 transition cursor-pointer border-none"
                    disabled={isLoading}
                  >
                    <X size={12} />
                  </button>
            ...
            <p className="text-[10px] text-slate-500 text-center py-1 font-semibold">
      ...
      <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200/80">
        {onCancel && (
          <button
            ...
            onClick={onCancel}
            disabled={isLoading}
            className="px-4 py-2.5 rounded-xl text-xs font-black text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition border-none cursor-pointer"
          >
            Cancelar
          </button>
        )}
